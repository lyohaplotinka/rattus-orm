...
Используйте декоратор `@AttrField(value)` (или метод `this.attrField()`) для определения наиболее
...
(для JS - `this.stringField()`, `this.numberField()` и `this.booleanField()`).
...
Как и в случае с `this.attrField()`, аргументом является
...
#### UID тип
`@UidField()`  (или `this.uidField()` в JS) генерирует уникальный
